...
var dateParser = function dateParser(val) {
  if (val) {
    ...
    if (dates.length) {
      ...
      var from = parseInt(dates[1].replace(':', '').trim(), 10);
      var to = parseInt(dates[3].replace(':', '').trim(), 10);
...
var getEventsContent = function getEventsContent(callback) {
  return request('http://gakkon.pl/atrakcje/spis-atrakcji/', function (error, response, body) {
