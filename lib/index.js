...
var importFestival = function importFestival(name, template, data, callback) {
  ...
  getFestival(festivalsClient, name, function (err, festival) {
    ...
    template.handle(data, function (err, templateData) {
      if (err) {
        return callback(err);
      }
...
var importNews = function importNews(name, template, data, callback) {
  var festivalsClient = createClient();
  ...
  getFestival(festivalsClient, name, function (err, festival) {
    ...
    template.handle(data, function (err, templateData) {
      ...
      var festivalId = festival.id;
      newsImporter.importNews(festivalId, templateData, function (err, result) {
        if (err) {
          return cb(err);
        }
