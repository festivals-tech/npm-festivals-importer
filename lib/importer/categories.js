...
var CategoriesImporter = function CategoriesImporter(festivalsClient) {
  var categories = {};
  ...
  var CategoryResolver = {
    ...
    resolve: function (category, festivalId, cb) {
      ...
      if (self.getCategory(category.name)) {
        return cb(null, self.getCategory[category.name]);
      }
      ...
      festivalsClient.getCategories(festivalId, query, function (err, response, body) {
        ...
        if (response.statusCode != 200 || body.total === 0) {
          ...
          if (category.parent) {
            var parent = self.getCategory(category.parent);
          ...
          festivalsClient.createCategory(festivalId, data, function (err, response, body) {
            ...
            if (response.statusCode === 201 && body) {
              self.addCategory(category.name, body);
              return cb(null, body);
            }

            return cb(new Error('Failed to create category: ' + data));
          });
        }
        else {
          self.addCategory(category.name, body.categories[0]);
